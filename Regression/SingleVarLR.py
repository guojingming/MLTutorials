...
class Svlr:
    def __init__(self, trainingSet, thresholdVariance = 20, maxIterationCount = 200):
        self.__J = 0
        self.__k1_s = sy.symbols("k1")
        self.__k2_s = sy.symbols("k2")
        self.__k1_v = 0
        self.__k2_v = 0
        self.__h = self.__hypothesis(self.__k1_v, self.__k2_v, sy.symbols("x"))
    ...
    def __BGD(self, currentIterationCount):
        ...
        if variance > self.__thresholdVariance and currentIterationCount < self.__maxIterationCount :
            ...
            self.lastestResult = {"k1" : self.__k1_v, "k2" : self.__k2_v, "variance" : variance, "iterationCount" : currentIterationCount, "h(x)" : self.__hypothesis(self.__k1_v, self.__k2_v, sy.symbols("x"))}
    ...
    def train(self):
        ...
        self.__J_k1_diff = sy.diff(self.__J, self.__k1_s)
        self.__J_k2_diff = sy.diff(self.__J, self.__k2_s)
        self.__h = self.__BGD(0)

    def predict(self, x):
        return self.__h.subs(sy.symbols("x"), x)